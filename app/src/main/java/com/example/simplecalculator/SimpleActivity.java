...
public class SimpleActivity extends AppCompatActivity {
    ...
    public void equals(View view){
        ...
        if(!expression.checkSyntax()){
            Toast.makeText(SimpleActivity.this, "Syntax Error!",
    ...
    public void sign(View view){
        ...
        SimpleActivity.dataType dataChange;
        if(Character.isAlphabetic(display.getText().toString().charAt(i)))
            dataChange = SimpleActivity.dataType.EXPRESSION;
        else
            dataChange = SimpleActivity.dataType.NUMBER;

        if(dataChange == SimpleActivity.dataType.NUMBER){
            for(; i > 0; i--){
                if(!Character.isDigit(display.getText().toString().charAt(i)) &&
